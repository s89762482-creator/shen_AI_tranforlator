const Translator = {
    ...
    _initSocket() {
        try {
            this._socket = io(CONFIG.API_BASE_URL, {
                reconnection: true,
            ...
            this._socket.on('connect', () => {
                console.log('[Translator] Socket.IO 已连接, ID:', this._socket.id);
            });
            ...
            this._socket.on('reconnect', (attempt) => {
                console.log('[Translator] Socket.IO 重连成功, 尝试次数:', attempt);
            });
            this._socket.on('reconnect_failed', () => {
                console.error('[Translator] Socket.IO 重连失败');
            });
            
            // 等待连接建立
            if (!this._socket.connected) {
                console.log('[Translator] 等待 Socket.IO 连接...');
                this._socket.on('connect', () => {
                    console.log('[Translator] Socket.IO 已连接，可以发送消息');
                });
            } else {
    ...
    // 等待 Socket.IO 连接
    async _waitForSocket() {
        if (!this._socket) {
            console.warn('[Translator] Socket.IO 未初始化，等待初始化...');
            await this.init();
        }
        
        if (!this._socket.connected) {
            console.log('[Translator] Socket.IO 未连接，等待连接...');
            return new Promise((resolve) => {
                const timeout = setTimeout(() => {
                    console.warn('[Translator] Socket.IO 连接超时');
                    resolve(false);
                }, 5000);
    ...
    _broadcast(type, data = {}) {
        // 只通过 HTTP API 发送字幕消息（更稳定）
        if (type === 'original' || type === 'translation') {
            const text = data.text || '';
            console.log('[Translator] 发送字幕 (HTTP):', { type, text });
            API.broadcastOverlay(type, text).then(result => {
                console.log('[Translator] 字幕发送成功:', type, text);
            }).catch(err => {
                console.error('[Translator] 字幕发送失败:', err);
            });
        } else if (type === 'translation-done') {
            // translation-done 不需要发送
    ...
    async processAudio(audioBlob, sourceLang = null) {
        ...
        console.log('[Translator] 处理音频，源语言:', lang);
        
        try {
            const result = await this._recognizeWithBackend(audioBlob, lang);
            console.log('[Translator] 识别返回结果:', result);
    ...
    async _recognizeWithBackend(audioBlob, sourceLang = 'en') {
        try {
            console.log('[Translator] 开始语音识别，源语言:', sourceLang);
            const result = await API.transcribe(audioBlob, sourceLang);
            console.log('[Translator] 识别结果:', result);
            if (result.success && result.data.text) {
                ...
                console.error('[Translator] 识别失败:', result);
            }
        } catch (error) {
            console.error('[Translator] 识别异常:', error);
        }
        return null;
    },
    ...
    _setPendingTimeout() {
        ...
        this._pendingTimeout = setTimeout(() => {
            console.log('[Translator] 超时，强制翻译缓存的文本:', this._pendingText);
            if (this._pendingText.trim()) {
                this._translatePendingText();
            }
        }, 5000);
    },
    ...
    async _translatePendingText() {
        if (!this._pendingText.trim()) return;
        
        const pendingText = this._pendingText;
        this._pendingText = '';
        this._clearPendingTimeout();
        
        console.log('[Translator] 强制翻译:', pendingText);
        
        if (this.onOriginalText) this.onOriginalText(pendingText);
        this._broadcast('original', { text: pendingText });
        ...
        await API.translateStream(
            ...
            () => {
                ...
                this.addHistory(pendingText, this._currentTranslation);
                if (this.onTranslatedDone) this.onTranslatedDone(this._currentTranslation);
                this._broadcast('translation-done', {});
            },
            (error) => {
