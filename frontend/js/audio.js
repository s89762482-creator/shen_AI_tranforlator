const AudioCapture = {
    ...
    async start(source = 'system') {
        ...
        try {
            ...
            // 优先使用 AudioContext 方法，直接生成 Vosk 兼容的 WAV 格式
            console.log('[Audio] 使用 AudioContext 录音模式（Vosk 兼容）');
    ...
    _useAudioContextRecording() {
        ...
        const originalSampleRate = track.getSettings().sampleRate || 48000;
        const targetSampleRate = 16000; // Vosk 需要的采样率
        
        console.log(`[Audio] 原始采样率: ${originalSampleRate}Hz, 目标: ${targetSampleRate}Hz`);
        ...
        processor.onaudioprocess = (event) => {
            ...
            // 每 3 秒生成一个 WAV
            if (pcmChunks.length >= Math.ceil(targetSampleRate * 3 / 4096)) {
                ...
                const wav = this._pcmToWav(combined.buffer, targetSampleRate);
